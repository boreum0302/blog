...
트리(Tree)는 연결리스트와 달리 노드들이 위아래로 연결된 계층적인 자료구조이다. 특히, 각각의 노드 아래에 두 개 이하의 노드가 연결된 이진트리(Binary Tree)가 컴퓨터 분야 전반에서 널리 활용된다. 하나의 예시가 힙(Heap)으로, 힙은 $$O(log N)$$이라는 빠른 시간에 가장 높은 우선순위를 가진 항목을 삭제하는 연산과 임의의 우선순위를 가진 항목을 삽입하는 연산을 제공하는 유용한 자료구조이다. 이진트리와 힙에 대해 정리했다.
