...
|이중연결성분(biconnected component)|(주어진 무방향 그래프에서) 단절 정점이 존재하지 않는 연결성분|
|강연결성분(strongly connected component)|(주어진 방향 그래프에서) 임의의 두 정점 `v1`과 `v2`에 대해 `v1`에서 `v2`으로 가는 경로와 `v2`에서 `v1`으로 가는 경로가 존재하는 연결성분|
...
|용어|정의|
|:--|---|
|인접행렬|정점 `i`와 `j`를 잇는 간선이 존재하면 `A[i][j]`의 값은 1이고 아니라면 0임|
