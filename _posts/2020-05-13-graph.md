...
그래프를 나타내기 위해 일반적으로 인접행렬(adjacency matrix) 혹은 인접리스트(adjacency list)가 사용된다. $$N$$개의 정점을 가진 그래프의 인접행렬을 2차원 리스트 `A`에 저장하는 경우에는, 정점 `i`와 `j` 사이를 잇는 간선이 존재하면 `A[i][j]`는 1이 되고, 존재하지 않는다면 `A[i][j]`는 0이 된다. 반면, 인접리스트를 1차원 리스트 `A`에 저장하는 경우에는, 
