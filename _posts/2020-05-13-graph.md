...
이때 사이클이 생기는지를 검사하기 위해 서로소 집합(disjoint set) 자료구조를 사용한다. 임의의 두 집합을 선택했을 때 교집합이 공집합이 되는 집합들이 서로소 집합 자료구조를 구성한다. 서로소 집합에 존재하는 모든 집합들의 원소가 $$0, 1, 2, \cdots, N - 1$$이라면 서로소 집합은 길이가 $$N$$인 리스트 `disj_set`에 저장할 수 있다. 하나의 집합을 하나의 트리로 나타낸 뒤에 `parent[i]`의 값을 `i`의 부모로 하는 일이 가능하기 때문이다(`i`가 루트일 경우 `parent[i]`의 값은 `i`임). 위와 같은 서로소 집합 자료구조의 경우 `{2, 1, 3}`, `{5, 4}`, `{6}`, `{7}`이라는 집합으로 구성되며 `parent`는 `[None, 2, 2, 2, 5, 5, 6, 7]`이다.  

```python
def Kruskal(graph, N):  # graph는 (정점1, 정점2, 가중치)의 리스트, N은 정점의 개수
...
```
    ...
    parent = []  # 서로소 집합을 나타낼 리스트
    for i in range(N):  # 아직 트리에 포함된 정점이 없으므로
        parent.append(i)  # 각각의 정점의 루트는 자기 자신임
        
    mst = []  # 트리에 포함시킬 간선들이 담길 리스트
    cost = 0  # 트리에 포함된 간선들의 가중치 합
    while True:
        if len(mst) == N - 1:  # 간선이 N - 1개가 되면
            break  # 실행 중지하기
