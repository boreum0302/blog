...
위상 정렬(topological sort)은 사이클이 없는 방향 그래프(directed acyclic graph)에서 각 정점들을 간선의 방향을 거스르지 않게끔 나열한다. 깊이우선탐색에서와 달리 정점 `v`에서 출발하여 달할 수 있는 정점들을 전부 방문한 후에 정점 `v`를 리스트에 담는다. 이때 정점 `v` 이전에 리스트에 담긴 정점들은 전부 정점 `v`에서 출발하여 달할 수 있는 정점들이 되기에 리스트를 역순으로 출력하면 위상 정렬을 할 수 있다.
...
임의의 정점에서부터 너비우선탐색을 한다면 인접한 정점을 전부 방문한 다음에 그 인접한 정점들에 인접한 다른 정점을 전부 방문하는 방식으로 탐색이 일어난다. 트리에서의 레벨순회와 동일하다.
