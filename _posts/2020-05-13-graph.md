...
하나의 연결성분으로 이루어진 무방향 가중치 그래프에 대해 간선들의 가중치 합이 가장 작은 신장 트리를 최소 신장 트리(minimum spanning tree)라 한다. 최소 신장 트리를 찾기 위한 `Kruskal` 알고리즘과 `Prim` 알고리즘은 탐욕(greedy) 알고리즘의 일종이어서 욕심내어 선택한 지역적인 최적해를 누적해 전체적인 최적해를 얻는다.
...
이때 사이클이 생기는지를 검사하기 위해 서로소 집합(disjoint set) 자료구조를 사용한다. 임의의 두 집합을 선택했을 때 교집합이 공집합이 되는 집합들이 서로소 집합 자료구조를 구성하며 존재하는 모든 집합들의 원소가 $$0, 1, 2, \cdots, N - 1$$인 경우에 서로소 집합은 길이가 $$N$$인 리스트 `parent`에 저장할 수 있다. 각각의 집합을 각각의 트리로 나타낸 뒤에 `parent[i]`의 값을 `i`의 부모로 하면 가능하기 때문이다(`i`가 루트일 경우 `parent[i]`의 값은 `i`임). 위와 같은 서로소 집합 자료구조의 경우 `{2, 1, 3}`, `{5, 4}`, `{6}`, `{7}`이라는 집합으로 구성되며 `parent`는 `[None, 2, 2, 2, 5, 5, 6, 7]`이다.  
...
서로소 집합에서 수행할 수 있는 연산에는 `find`와 `union`이 있다. `find(v)`는 `v`가 속한 집합의 루트를 반환한다. 이때 루트까지 올라가며 만나는 노드의 부모를 루트로 갱신하는 과정인 경로 압축(path compression)이 일어나는데 이는 추후 실행되는 `find` 연산의 수행 시간을 단축한다. `union(v1, v2)`는 `v1`과 `v2`가 속한 집합을 합친다. `v2`가 속한 집합의 루트의 부모를 `v1`이 속한 집합의 루트로 갱신해 주면 된다.
...
주어진 그래프에 $$N$$개의 정점이 존재한다 하자. 임의의 정점 `start`만을 가지는 초기 트리에서 시작하여 트리에 속한 정점의 수가 $$N$$이 될 때 까지 트리에 속한 정점들에 인접한 정점들 중에서 트리에 속하지 않는 동시에 가중치가 가장 작은 간선으로 연결된 것을 트리에 추가하는 작업을 반복한다.
