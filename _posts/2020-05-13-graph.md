...
이때 사이클이 생기는지를 검사하기 위해 서로소 집합(disjoint set) 자료구조를 사용한다. 서로소 집합 자료구조는 임의의 두 집합을 선택했을 때 교집합이 공집합이 되는 집합들과 같다. 서로소 집합에 존재하는 모든 집합들의 원소가 $$0, 1, 2, \cdots, N - 1$$이라면 서로소 집합은 길이가 $$N$$인 리스트에 저장할 수 있다. 
