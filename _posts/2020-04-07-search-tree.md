...
좌편향 레드블랙트리는 아래의 조건을 만족하는 이진탐색트리이다. 2-3 트리에서 3-노드에 있는 2개의 키를 2개의 노드로 분리하여 저장한 뒤 왼쪽을 레드 오른쪽을 블랙으로 칠한 형태와 동일하다 볼 수 있다.

- 루트와 `None`은 블랙임
- 루트로부터 각각의 `None`까지2개의 연속된 레드 `link`가 존재할 수 없음
- 루트로부터 각각의 `None`까지의 경로에 있는 블랙 `link`의 수는 전부 같음
- 레드 `link`는 왼쪽으로 기울어져 있음

## B-트리
B-트리는 노드에 수백에서 수천 개의 키를 저장하여 높이를 낮춘 트리이다. 아래의 조건을 만족하는 트리를 차수가 M인 B-트리라 한다. 키의 삽입에는 분리 연산이, 키의 삭제이는 이동 연산과 통합 연산이 사용된다.
