...
구글링 하다가 잘 안 나와서 스스로 코드를 짰는데, 시간이 $$O(N^2)$$이나 걸리는 참사가 일어났다. `while` 절에서는 레벨순회를 하면서 `item`을 복사한 뒤 `q`에 노드의 형태로 집어넣는다.  `i = i + 1`을 같이 해준 이유는 노드의 개수를 세기 위해서이다. 이제 `for` 절에서는 `q`에 복사된 노드들을 원래 트리에서처럼 연결해준다. 마지막에 `BinaryTree()` 객체를 생성하고, 이 객체의 `root`를 `q[0]`으로 설정한 뒤 반환하면 끝난다.
