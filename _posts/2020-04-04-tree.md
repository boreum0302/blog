...
이진힙은 힙 속성을 만족하는 완전이진트리로 정의한다(힙 속성은 각각의 노드에 대해 부모의 우선순위가 자식의 우선순위보다 높은 속성임). 각각의 노드에 대한 우선순위는 위에서 용어 정리를 할 때 등장했던 키(Key)의 값이 결정한다! 이진힙에서 키의 값이 작을수록 우선순위가 높은 경우 최소힙(Minumum Heap)이라 하며, 키의 값이 클수록 우선순위가 높다면 최대힙(Maximum Heap)이라 한다. 여기서는 최소힙으로 이진힙을 설명하겠다.
